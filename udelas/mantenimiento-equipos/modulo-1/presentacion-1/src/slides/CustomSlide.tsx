import { SlideLayout, SlideTag, SlideTitle, Card, Frost } from './SlideLayout'
  ...
  return (
    <SlideLayout style={{ alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      ...
      <h2
        ...
        style={{ fontSize: 54, lineHeight: 1.14, color: 'var(--ice)', margin: '0 0 18px', fontWeight: 400, maxWidth: 900 }}
      >
      ...
      <div
        style={{
          ...
          boxShadow: '0 0 12px var(--frost)',
        }}
      />
      <p className="stagger-item" style={{ fontSize: 17, color: 'var(--ice-dim)', margin: 0, maxWidth: 640, lineHeight: 1.7 }}>
        ...
        {range(items).map(i => (
          <div
            ...
            style={{
              ...
              animation: `edgeSlide 0.45s cubic-bezier(0.16,1,0.3,1) ${0.08 + i * 0.08}s both`,
            }}
          >
            <span
              className="font-mono"
              style={{ fontSize: 13, color: 'var(--frost)', flexShrink: 0, paddingTop: 1, minWidth: 22 }}
            >
              {String(i + 1).padStart(2, '0')}
            </span>
            <p style={{ fontSize: 14, color: 'var(--ice-dim)', margin: 0, lineHeight: 1.65 }}>
  ...
  return (
    <SlideLayout>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18, flex: 1, minHeight: 0, justifyContent: 'center' }}>
        <p className="stagger-item" style={{ fontSize: 16, color: 'var(--ice-dim)', margin: 0, lineHeight: 1.8, maxWidth: 900 }}>
          <span>
        ...
        <Card style={{ maxWidth: 900 }}>
          <p className="font-mono" style={{ fontSize: 12.5, color: 'var(--frost)', margin: 0, lineHeight: 1.7 }}>
...
function TwoColTemplate({ items }: { items: number }) {
  return (
    <SlideLayout>
      <SlideTag><span>Sección</span></SlideTag>
      <SlideTitle size="md">
        <span>Comparación</span> <Frost><span>lado a lado</span></Frost>
      </SlideTitle>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, flex: 1, minHeight: 0 }}>
        {(['frost', 'rift'] as const).map((tone, col) => (
          <Card key={tone} tone={tone} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <p
              className="font-display"
              style={{ fontSize: 22, color: tone === 'frost' ? 'var(--frost)' : 'var(--rift-soft)', margin: 0 }}
            >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {range(items).map(i => (
                <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span
                    style={{
                      width: 6, height: 6, flexShrink: 0, marginTop: 7,
                      clipPath: 'polygon(50% 0, 100% 50%, 50% 100%, 0 50%)',
                      background: tone === 'frost' ? 'var(--frost)' : 'var(--rift-soft)',
                    }}
                  />
                  <p style={{ fontSize: 13.5, color: 'var(--ice-dim)', margin: 0, lineHeight: 1.6 }}>
